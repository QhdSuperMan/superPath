  ...
  <div class="list-common-style-container">
    ...
    <div class="table-list">
      <div class="table-list-btns">
        <CustomButton icon="delete" v-if="btnAuthority.s_transfer" size="small" @click="batchOperation(6)"
          >转交</CustomButton
        >
        <CustomButton icon="delete" v-if="btnAuthority.s_release" size="small" @click="batchOperation(2)"
          >释放
        </CustomButton>
        <CustomButton icon="delete" v-if="btnAuthority.s_release_all" size="small" @click="releaseAllCustomer"
          >释放全部
        </CustomButton>
      </div>
      <CustomTable
...
import { getList } from "@/api/customer/customer.js";
